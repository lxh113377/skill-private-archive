# 05 - 功能状态

> 本文件记录功能/任务的完成状态。
> 归档类型：增量（已完成项归档时移入 archive/）

## ✅ 已完成

- [x] **r58 供应链安全面（第 17 维）对标 + 三项同类缺陷根因修**（2026-09-26，本项目 `06-benchmark/全量对标报告_r58_*.md`）：① 新维度实测入册（`05-exec/r58_supply_chain_face.py` → `supply_chain_face_r58_2026-09-26.json`，两路对账：`.github/workflows` contents 目录面 × `/actions/workflows` API 总计面）；② **D-113** 推翻 r31/r32「本仓取 tag」既定裁定，两行 action pin 到实测 SHA（不跨大版本）+ 新建 `dependabot.yml` + 新门 `action_pin_guard`（P1 形态/P2 pin 必配更新器/P3 空面不判过），桩 11/11（含真仓对照组与变异体）；③ **D-117** 自抓 `evidence_resolvability_measure.py:111` 少右括号致该工具**入库以来从不可运行** ⇒ 新门 `py_syntax_guard`（99/99 + 反例 + 空面三向）；④ **D-115** 实测推翻 r57 的 `trim-shell` 药方并分治收口 TODO.md（31,685B→11,883B，11 节原样迁 `memory/todo-history.part1.md`，精确多重集行守恒 0/0，备份 `_trash/r58_todo_slim_*`）；⑤ **W-46 误报率量清并判 HOLD**（30.6%，真死句柄 1 条）；⑥ **D-114** `comparison.md` 对手名写错致 404，加 R241 校正注；⑦ 契约 16→18 pattern，末态 `[GATES:PASS] 23` / `[CONTRACT:PASS] 44 份` / `[RATCHET:PASS] 9`

r57 体量治理入工作流轮（2026-09-25 第 27 轮，指令驱动轮）— 用户令升级 A-project-handoff / A-project-better，把「项目工作空间文件体积」纳入动态工作流防无限膨大。① **判据实现**（受管根 `f3c82ed` V3.57.0）：新增 `handoff_lib/volumegov.py` 四层扫描（L1 记忆卷复用 `volume_health()` 同源、L2 根文档注入面、L3 产物/目录/整仓、L4 备份残留簇）+ 三层阈值 + 增长率快照（`memory/sessions/volume-snapshots.jsonl`）+ 新子命令 `volume`；② **与工作流节点结合**：`flow --volume --emit-tasks` 把待判断项登记为 09 任务（id 前缀 `VOL-`，按 id 幂等）→ `--sync` 回写 07；`flow --status`/`--check`/`savepoint` 三处加体量摘要行（只读快照零扫描）；③ **advisory 口径**（用户裁定）：warn 恒退 0，自动动作仅两类（拆卷/归档 + `recycle` 入回收站禁永久删），L2/L3 交付物只出命令不出手；④ **阈值落点**：代码 `DEFAULT_BUDGET` 单一数据源，项目可在 06 节写 `- 体量预算 k=v` 覆盖、`--set` 一次性放宽，生效值必须回显；⑤ **消费端**（A-project-better **V1.9.0**）：Step1 体检必跑、四维→**五维**（加 E 体量）、Step5 体量收口，细则入新卷 `workflow-volume.md`（各卷 ≤4096B）；⑥ **验证**：判据桩 `scripts/volume_gov_stub.py` **22/22**（10 处反例含「零输入不记 PASS」「L2 不误报」「二次跑幂等」）+ 真机本项目 `--apply` 实测释放 **2,679,685B**（7,520,744B→4,841,059B，20 份 `.bak` 入系统回收站并写 `_trash/recycle_manifest.jsonl`），登记 3 项 VOL + 3 项推荐进 09/07，`flow --check` 由 3 红 → `[GATE:flow-check-pass]`；`rule_editor gates` 四项 mirror/noise/evolution/stub 全绿；⑦ **自抓两坑**：只看 `gates` 聚合输出致「他人脏项」误归因（明细在判据脚本本体里，13 项全是自己的）；未过 `rule_editor` 通道改受管根文件 ⇒ 触发 R269 脏源 fail-closed（逐 hunk 归属后 `--allow-dirty-source` 放行并留痕）

r56 声明可校验轮（2026-09-25 第 26 轮，批注驱动轮非重复触发轮） — W-37 再生预算入基线件 `refresh_days`（v1→v2）+ 契约集合全等 + `--update` 不得改写预算 + 缺预算禁回落代码常量（b0-b9 十例）；W-38 `refresh_stale_count` 进台账（分代点 19:15，取不到记 None）并把代际口径推广到 JSON 件（`required_from`/`invariants_from`，豁免必须打印否则=隐形豁免表；e1-e10 十一例）；R-ENUM 第 4 形态「保质期断言」落规受管根 `b2e3f4e`（governance.md 单文件 + 定向镜像同步 + hash 复检，同技能 SKILL.md/version-history 被并行会话占用故不夹带 → W-45）；自抓 W-27/W-32 两份裁定件无生成器=死句柄，补 3 个生成器（同枚举器 + 逐件求和交叉核对，变异 7B 即 DRIFT）纳入契约并立 W-46；对手 CI 双路对账：自有 workflow 18/0/0/1/2 vs api 总计面 27/2/2/5/3（五家差值全非零，两家目录 404 而 api 报 2，truncated 已排除）⇒ 第 8 维改双列；常驻门 19→21、`[RATCHET:PASS] 9`、`[CONTRACT:PASS] 42 份/16 pattern`、活体引用面 281/0
r55 输入面根因收口轮（2026-09-25 第 25 轮，指令驱动轮非重复触发轮） — ① `memory/AGENTS.md` 排障速查表落地并自抓 row 1 首版未实测缺陷；② 根因＝同一判据两个输入面：`noise_lint.KNOWN_CORE` 缺本项目根 ⇒ 传项目根即 strict 全判 VIOL（顶层 8 条假阳性），而 `savepoint` 第 i) 步 `cmd_noise(["--quiet"])` 不带项目根参数 ⇒ 只扫四受管根（「零散射」对项目根是空承诺）；修法按 R281 先例走登记不放宽判据 = 受管根 `e93896b`（R286），复验顶层 VIOL 8→0、二级随即露出 3 条真项，清自家 `_tmp_r54` 后保留 2 条诚实红（R269 项 + W-39 项，禁 `.gitignore` 凑绿）；③ 镜像缺口：`rule_editor commit` 不同步镜像 ⇒ `mismatch=5`（3 个归我、2 个是并行会话**未提交**源码），刻意不按全局 `-Fix`（铁律 8 单向覆盖），只 `cp -p` 自身 3 文件 + hash 复检（5→2），配方固化为排障表 row 14；④ 新开第 15 维对标：0/5 对手存在「需同步的第二份技能树」，我方镜像有 10 个「仅在镜像」文件 ⇒ junction 非免费午餐（W-42）；⑤ 自抓 `ratchet_gate.py --update` 整份重建基线件把 note 496→33 字、三条人工核定依据蒸发且棘轮照样绿 ⇒ 抽 `merge_note()`（只增不减/人工 note 不切片/缩短拒写）+ `git show HEAD:` 恢复 + 6 例夹具含变异体复验；新立 W-39/40/41/42（建议面 4/4 承接）；常驻门 17→19（`root_face_fixtures` 11 例、`baseline_note_fixtures` 6 例）；**二次自抓并修于受管根 `e5c785e`（R287）**：登记只治好绝对路径，`noise .` 相对写法当时仍判 13 项 ⇒ 查表键绝对化（显示与扫描根不动），f10/f11 双向锁，四受管根 violation 全 0 且镜像侧同结论
r54 再生预算轮（2026-09-25 第 24 轮） — `ratchet_gate` 新增 `METRIC_SOURCES`/`METRIC_REFRESH_DAYS`/`source_age_days`/`face_metric_refresh`/`refresh_status`：9 指标逐个声明保质期，STALE 转 unknown、UNVERIFIED 不转，CLI 打印「再生周期」行；真机误报率 0（源件年龄 0.0–0.99 天）；夹具 12 例（两向 + 配置形态反例）；第 5 维换口径：五家仅 2 家声明节律（12/7 行），且 pre-commit 39 天无提交却声明最密 ⇒ 节律≠活跃度，改三列并列；W-35 续款 README 24 链接 0 死链；新立 W-37（预算入契约）/W-38（再生健康度进趋势）；常驻门 16→17
r53 句柄化引用轮（2026-09-25 第 23 轮） — `make_handles`/`resolve_handle` 落地于 `05-exec/r38_debt_aging.py`（句柄四件齐、解不出返 None 不返 0、h7 写读闭环守恒）+ 第 16 道常驻门 `handles_readme_fixtures`；W-31 陈旧自检改 mtime 优先（打掉首版 3 个命名假阳性，t33/t34 两向锁死），真机陈旧面 3→0；README 补证据面导航 19 链接/死链 0（对照 AAS 50 链接，我方此前为 0）⇒ 立 W-35 续改产物地图；立 W-36（指标须声明 max_age_days，超期判 unknown）；重跑 rubric 与目录税两探针；W-33/W-34 裁决回写
r52 引用面对账轮（2026-09-25 第 22 轮） — 新工具 `05-exec/r52_md_claim_face_scan.py`（md 数字须同节 ±12 行带取值命令，窗口经 14 条抽样判读确定）+ 第 15 道常驻门 `md_claim_face`；可整改面 184 声明 / 0 敞口、历史面 755 处只登记；`face_pattern_staleness`（W-31）抓到 3 个陈旧 pattern 含轮号写死形态（W-33）；对手第 6 维实测（正文数字 0–1 / 徽章 1–5）⇒ 立 W-34「报告数字改引用台账字段」；W-30/31 执行完毕、W-32 转裁定；夹具 20 + 46 例全绿
r51 取数面对账轮（2026-09-25 第 51 轮） — W-29 复核：我方引用的对手 open issues 含 PR 虚高 1.5–3.5 倍（134/143/368/58/17 为纯 issue 面），日期列 5/5 复现 ⇒ 命令须逐条；W-28 落 `inv_opponent_claims_have_retrieval`（结构识别不锁键名 + claims_face + 双路命令），并抓出 arg 传名字串导致全量豁免的接线缺陷（立 X-28，反例 r11/r12）；契约陈旧 glob `debt_aging_r3*`→`r*`；r50 非法 JSON 取证件重建并纳管 `opponents_*_r*.json`；W-27 分级 21 组=9 词（立 X-27）；W-19 以 `--mode note` 自用收口；常驻门 13→14
r50 扩面与复核轮（2026-09-25 第 50 轮） — W-25：`05-exec/rule_conflict_scan.py` 扫描面 10→26 件（自动发现带极性规则的权威源分卷，极性规则 116→188、互斥候选 6→21）+ `05-exec/ratchet_gate.py` 读取面交叉对账；W-26：混合行尾根因用 `git show` 定死为写卷未传 `newline=""`（06 加归因校正注，立 X-25）；对手取证口径校正：superpowers/anthropics 仓库内 0 CI，`06-benchmark/opponents_workflow_face_r50_2026-09-25.json` 四路取值留证；建议面按前缀分路由（X→AGENTS 禁止项表）修掉自造假阳性，夹具 r49_face 32 例、r50_face 16 例，常驻门 12→13
r49 输入面五面自证轮（2026-09-25 第 49 轮） — W-20：`05-exec/r38_debt_aging.py` 新增 face_round/face_floor/face_dedup/face_dating/face_suggestions 五面（判据读的面必须自证未被截断/降采样，含 R-ENUM 要求的 floor 集合断言），`face_status` 进台账；W-22/23/24：写入器第⑥重对账 + 混合行尾逐行保留 + 分类器只认标记头部，`05-exec/r49_face_fixtures.py` 25 例、`r39_debt_ledger_fixtures.py` 58 例（含 t70–t72 台账列、t63/t64 多代必填）、`r46_mark_verdict_fixtures.py` 10 例（t9/t10 混合行尾反例）全绿；5 条到期债 OVERDUE 5→0、DECIDED 60→70；常驻门 11→12（**同轮补记 r49**：定年面随后按权威契约 R-ENUM 边界条改为「零到期项 ⇒ 该面未行使」，故末态为 `[FACE:UNVERIFIED]` 而非五面全绿 —— 没检验过不许写成检验通过；夹具加 f14–f16 三态锁死，台账列 face_status 如实记 UNVERIFIED）
r48 契约分代与归属可机检轮（2026-09-25 第 48 轮） — W-17：契约 row_required_from 按 ts 分叉强制 coverage_* 三列（新行缺即红、历史行豁免），解掉 r45 自挂的两难；W-6：classify_owner + owner 字段 + unowned_claims，声明归属却无可解析路径 ⇒ UNOWNED（动因 r40 D37 自家债外部化挂 4 轮）。D75 自抓：owner 最初用截断 title[:110] 判，3 条 UNOWNED 里 2 条是假的 ⇒ 改判全文；剩 3 条真项逐条经 r46 写入器补路径后归零。D76 一次 Edit 把字典尾部写重复致 SyntaxError（ast 当场拦下）。夹具 53/53、契约 13 pattern/17 件 PASS、[GATES:PASS] PASS=11、W-12 OK。立 X-22。

r47 盲区清零轮（2026-09-25 第 47 轮） — 纯还债轮：W-1 十二条 UNDATED 逐条 `git log -S` 取真实首现、逐条经 r46 写入器一次写「定年 + 终局裁决」⇒ UNDATED 12→0、DECIDED 58、OVERDUE 0、REPEAT 0、W-12 OK。限定：6 条定到轮号、6 条只定到日期（如实写 r?）。处置 4 作废 / 6 保留 / 2 降级看守，无一条凭感觉划掉。W-18 dogfood 12/12 经工具、备份不落记忆卷、零行号错写。D74 记半成品命令之失。立 X-21（禁以裁决标记冒充定年）。

r46 标记写入唯一入口轮（2026-09-25 第 46 轮） — W-16 固化为 `r46_mark_verdict.py`：不接收行号、歧义/0 命中/已闭环/同轮二次一律拒写、写完 grep 读回否则回滚、备份可 --backup-dir 注入。真实卷冒烟即拒写歧义锚点 [87,90]（W-9 挂 5 轮的机制学解释）。夹具 8/8，注册为第 11 道常驻门 ⇒ [GATES:PASS] PASS=11；dogfood 裁掉 W-6 ⇒ OVERDUE 1→0、DECIDED 46、REPEAT 0。三处自抓：备份目录写死致不可测（改依赖注入）、又写弱断言 t9（删除）、用真实卷冒烟误写一条标记（当场回滚 + .bak 迁出记忆卷）⇒ 立 X-20（禁拿真实记忆卷做冒烟）、W-18（补标一律走本工具）、W-19（05/06/08 回写同入口）。W-15 只做一半如实挂账 W-17（契约分代必填），未虚报完成。

r45 反降级免检轮（2026-09-25 第 45 轮） — W-14 落地棘轮第 9 指标 repeat_debt_items（共用自洽门；动因 D59：r44 用 11 条降级把 OVERDUE 压到 1，降级无独立指标即成新免检通道）；TDD 先红 t50-t55 六例后 45/45，并删掉自己刚写出的恒真断言 t53。W-15 部分落地：台账行加 coverage_status/prev_overdue/missing 三列；未加进契约 row_required 的理由如实记录（会让 r38-r44 历史行全体违规且行不可回写）⇒ 立 W-17 按 ts 分代必填。W-9 真因归位：H-3 项裁决标记数为 0，证明我前几轮按行号写标记写到别处 ⇒ 立 W-16（临时脚本必须复用 find_item_line）。到期追讨第 3 次真响，3 条按 X-17 降级看守 ⇒ OVERDUE 3→0、DECIDED 46、REPEAT 0，W-12 完整性首报 OK。

r44 裁决完整性落地轮（2026-09-25 第 44 轮） — 落地连续两轮点名的 W-12/W-13：find_item_line（标题锚点、只认未勾选项、多命中拒写）+ adjudication_gap/coverage_check（四条语义由夹具 t40–t46 锁死），夹具 39/39。判据首跑即报 INCOMPLETE（上轮 15 条一条没裁），据此真裁决 15 条（11 降级看守 + 2 执行完毕 + 1 延期 r47）并补勾 r40 已交付漏勾的 W-4 ⇒ OVERDUE 16→2→1、DECIDED 43、REPEAT 0。余 1 条真实未做 ⇒ 棘轮如实留红，未上调基线。过程自抓：先发现自写测试互相矛盾（t44/t45 vs t43）再定语义实现，未改断言凑过。立 X-18（判据不当场跑真实数据不得宣布有效）、W-14（REPEAT 第 9 指标）、W-15（完整性进契约）。

r43 反延期落地轮（2026-09-25 第 43 轮） — 把 r42 的 4 条红测试落地：第 6 态 REPEAT（同行改期 >=2 次显形，RE_DEFERRAL 计数 + REPEAT_AFTER=2）与按优先级分档宽限 P0:2/P1:4/P2:8（值入 GRACE_BY_PRIORITY 常量表），台账行新增 repeat 字段；夹具 32/32。首跑显形 6 条滚延期项、分档使 OVERDUE 20→15；按 X-16 逐条升级：11 条判「降级为长期看守」（终局但数值仍被棘轮看守）、4 条首次延期至 r46，复测 OVERDUE 15 / DECIDED 24 / DEFERRED 4 / REPEAT 2。**棘轮如实留红 +15，未上调基线**。新发现 D55 行号漂移（5 条写入时已非未勾选行，守卫跳过未强改）→ 立 W-13 标题锚点定位、W-12 裁决完整性。

r42 根因轮（2026-09-25 第 42 轮） — 查清 OVERDUE 反弹真因：轮号推进致 15 条如期到期（14 计入）+ 上轮漏裁 r38 自立项（账龄新越线 6 条），**非判据缺陷、非正文替换扰动**；据此定性流程缺陷并写失败测试 t30–t34（REPEAT 反延期 treadmill + 分档宽限）。实现未落地（补丁中途断言失败未写盘），如实挂红 28/32，立 W-11 为下轮 P0。

r41 结构归位与 last-wins 轮（2026-09-25 第 41 轮） — 纠正 r40 误诊（到期洪峰是机制正常，非判据缺陷，D41）；抓到 W-8 真根因：多裁决标记同行时 `search()` 取最早 ⇒ 新终局被旧挂账劫持，改 `finditer()[-1]`，夹具 t26/t27 双向锁死 → 27/27；H-5 结构债清掉：P0 节内 15 条轮次摘要整段归位，守恒三校验全过（行数守恒、残留摘要 0、60 条待办未删）；再裁 9 条到期债（3 作废 + 6 挂账至 r44 各带取值），账面 OVERDUE 10→1 / DECIDED 12→15 / DEFERRED 25→22。如实留红：[RATCHET:FAIL] overdue=1，不使用归因通道掩盖，立 W-9 与 X-15。

r40 还债轮（2026-09-25 第 40 轮） — 不开新维度、专修判据放水：W-4 第 8 指标 deferred_debt_items（共用单源读取器、同等 fail-closed，24/24）、到期追讨首次真响 3 条、L-4 归属反转（扫描器原在本仓；契约新增 conflict_no_dead_inputs + 死条目改指真实在役件 + 第 10 道门 5/5）、M-1 判为历史不可测（git 不记脏树）改前瞻影子计数。自错三处留证并加代际豁免。立 X-14。如实留红：RATCHET overdue+1/deferred+2，W-8 列下轮 P0。

- **r21c 还账轮（2026-09-24 第 21 轮 c 段）** — 本仓欠焚诀 C25/C31 的注入超顶已清偿：`A-memory-start/SKILL.md` 29,041→26,593B（长条目换一行摘要，全文归分卷），其余量 220→1,583B；实测 owner 已自落 C31 归因台账 ⇒ 我「注入区合一」建议作废。受管根 `928517a`/V10.70.0。全文见 `05-feature-status.part19.md`。
- **r20b 棘轮落地轮（第 21 轮 b 段）** — A-get-memory 三条硬判据落地（受管根 `b8966da` V4.30.0，四门禁首次全绿）+ 本仓注入面棘轮 `ratchet_gate.py`（21/21 夹具，两级判定）挂进项目门禁命令。全文见 `05-feature-status.part18.md`。
- **r20 机制化轮（第 21 轮）** — rule_editor 写前脏源检测落地（受管根 `9a15f4e` V10.69.0，桩 10/10）+ 09 状态机首次真用 + comparison 常驻页。全文见 `05-feature-status.part17.md`。
- **r19b 标注驱动续做轮（第 20 轮 b 段）** — 契约族 + 变异对照 + 注入面端数 6 句改口八端（V10.68.0）。全文见 `05-feature-status.part15.md`。
- **r19 对标实物层增量轮（第 20 轮）** — 实物层校准 + 55 例夹具 + claim 对账新工具 + 分母三口径 + 上游 flow 图例缺陷修（V3.52.2）。全文见 `05-feature-status.part16.md`。
- **r18 全量对标增量轮（2026-09-24，第 19 轮）** — 对象 7→14 + 差距 N1~N8 + 4 只读工具落地；全文见 `05-feature-status.part14.md`（本轮条目级归档迁出，正文零改写）。⚠️ **r19 校正注**：该轮四工具当时**零夹具**、drift 扫描含 37.5% 噪声，已由 r19 修正。

## 分卷目录
- **卷1** `05-feature-status.part1.md` — 进行中 / 计划中 / 阻塞
- **卷2** `05-feature-status.part2.md` — 已完成：2026-09-19 第 3 轮（P0-2 收口 + 补建上游 + 注册表重建）
- **卷3** `05-feature-status.part3.md` — 已完成：2026-09-22 第 4 轮（D1–D5 授权项执行结果）
- **卷4** `05-feature-status.part4.md` — 05-feature-status 分卷（R199 自动拆卷）
- **卷5** `05-feature-status.part5.md` — 05-feature-status 分卷（R199 自动拆卷）
- **卷6** `05-feature-status.part6.md` — 已完成：2026-09-22 第 10 轮（Step 0 首次实跑 + 记忆层回写 + 阶段1 补齐 + B1–B7 授权执行）
- **卷7** `05-feature-status.part7.md` — 05-feature-status 分卷（R199 自动拆卷）
- **卷8** `05-feature-status.part8.md` — 已完成：2026-09-23 第 13 轮（性能瓶颈三维实测分析）
- **卷9** `05-feature-status.part9.md` — 05-feature-status 分卷（R199 自动拆卷）
- **卷10** `05-feature-status.part10.md` — 05-feature-status 分卷（R199 自动拆卷）
- **卷11** `05-feature-status.part11.md` — 05-feature-status 分卷（R199 自动拆卷）
- **卷12** `05-feature-status.part12.md` — 05-feature-status 分卷（R199 自动拆卷）
- **卷13** `05-feature-status.part13.md` — 05-feature-status 分卷（R199 自动拆卷）
- **卷14** `05-feature-status.part14.md` — 已完成：2026-09-24 第 19 轮 r18 全文（条目级归档迁出，防主壳超 4KB）
- **卷15** `05-feature-status.part15.md` — 已完成：2026-09-24 r19b 全文（契约族+变异对照+端数改口）
- **卷16** `05-feature-status.part16.md` — 已完成：2026-09-24 第 20 轮 r19 全文（自 part15 就地续拆）
- **卷17** `05-feature-status.part17.md` — 已完成：2026-09-24 第 21 轮 r20 全文
- **卷18** `05-feature-status.part18.md` — 已完成：2026-09-24 第 21 轮 r20b 全文
- **卷19** `05-feature-status.part19.md` — 已完成：2026-09-24 第 21 轮 r21c 全文
- **卷20** `05-feature-status.part20.md` — 05-feature-status 分卷（R199 自动拆卷）
- **卷21** `05-feature-status.part21.md` — 05-feature-status 分卷（R199 自动拆卷）
- **卷22** `05-feature-status.part22.md` — 05-feature-status 分卷（R199 自动拆卷）
- **卷23** `05-feature-status.part23.md` — 05-feature-status 分卷（R199 自动拆卷）
- **卷24** `05-feature-status.part24.md` — 05-feature-status 分卷（R199 自动拆卷）
- **卷25** `05-feature-status.part25.md` — 05-feature-status 分卷（R199 自动拆卷）
- **卷26** `05-feature-status.part26.md` — 05-feature-status 分卷（R199 自动拆卷）
- **卷27** `05-feature-status.part27.md` — 05-feature-status 分卷（R199 自动拆卷）
- **卷28** `05-feature-status.part28.md` — 05-feature-status 分卷（R199 自动拆卷）

