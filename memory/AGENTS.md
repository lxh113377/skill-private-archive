...
| X-8 | 补丁脚本只 `return` 新字符串就打印「done」 | **r36 立**：r35 的记忆回写**从未落库**却打印过 done（`git log -- memory/07-next-steps.md` 可证最后触碰者是 r34）。**回写后必须 grep 读回命中条数，才允许报完成**；改记忆类文件后还须跑 `git show --stat --name-only HEAD` 做归属核对 |
