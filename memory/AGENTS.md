...
| X-22 | 待办只写「属归属方 / 转办某会话」却不给**可解析文件路径** | **r48 立**：r40 实测我把本仓自持的 `rule_conflict_scan.py` 标成归属方件挂 4 轮 —— 甩锅比降级更便宜。`classify_owner()` 现把此类条目判 UNOWNED 并逐条打印 |
| X-23 | 用「全文 search 找标记」判定条目状态（裁决/延期计数） | **r49 立**：标记是 append 在同一行里的，原因体里**引用**旧标记原文（本轮实测写了「r46 却在本条上写了『挂账至 r49』」）会被 last-wins 解析劫持 ⇒ 终局裁决被回判成过期挂账（凭空造红）+ 延期计数虚增（REPEAT 失真）。口径 = 只认**最后一个标记的头部**（第一个 ｜/】 之前），原因体一律不参与判定；落 `parse_last_verdict/count_deferrals` + 负控 q1–q5 |
| X-24 | 枚举器（决定分母的代码）只做「面内自洽」校验 | **r49 立**（对齐权威契约 R-ENUM，`D:/global_skills/A-skill-manager/references/governance.md`）：glob 少收一卷、窗口截断、路径转义都会让分母静默变小而**所有面内不变式照样全绿**——「零积压」于是是在 1/16 语料上算的。必须有形状/下限/交叉三类反向断言之一；本仓落 `face_floor`（上轮受检卷须全部仍在本轮面内，集合断言故无需豁免表）+ f11–f13 夹具 |
| X-25 | 用 text 模式（未传 `newline=""`）写共享记忆卷 | **r50 立**：Windows 下写文件不传 `newline=""` 会把 LF 前补成 CR，而仓库内正形是 LF（实测 `git show r49:memory/07-next-steps.md` 全 190 行纯 LF），于是同一卷被逐轮改造成**混合行尾**；任何「文件里有 CRLF 就整份按 CRLF 切」的判据/写入器都会并元素、把内容写到相邻行（r49 实测错写）。一律 `newline=""` + 逐行保留原行尾 |
| X-26 | 把**手抄清单**当扫描分母（权威源列表写死在代码里） | **r50 立**：`rule_conflict_scan.DEFAULT_FILES` 手抄 10 件，而 `D:/global_memory/core` 有 18 件带可配对正负规则（规则分卷 `behavior_core_rules_p1..p8` 才是 22 锚点正文所在）⇒ 16 件从未进面；扩面后极性规则 116→188、互斥候选 6→21，**原绿灯只是 38% 面上的结论**。清单只作「已知核心」起点，须与按形状自动发现的候选取并集 + 覆盖面自证（漏扫即点名） |
| X-27 | 把**笛卡尔积展开出的候选数**当「冲突条数/问题条数」对外引用 | **r51 立**：`rule_conflict_scan` 的候选 = 同侧每个命中行两两配对（同一规则区 3 行含「必须含下一步建议」× 对面 2 行含禁语 ⇒ 记 6 组）。21 组实测只等于 **9 个不同共用词** ⇒ 引用时必须同时给去重维度口径，否则一次分卷复制就能把严重度放大三倍 |
| X-28 | 判据的 `arg` 直接当配置值用 | **r51 立**：`baseline_contract_scan.INVARIANTS` 派发处传的是**不变式名字符串**（`fn(doc, name)`）。我新写的豁免分叉点实现 `cut = arg if isinstance(arg,str) else 默认` ⇒ 名字串被当时间戳，`"2026-.." < "opponent_.."` 恒真，**全部证据件被字典序静默豁免**、判据当场失效而夹具（传 True）照样全绿。要可配参数必须显式校验形态（如正则匹配 ISO 时间戳），并补"按派发处真实传法"的接线反例 |
| X-21 | 用「加了裁决标记」冒充「已定年」 | **r47 立**：定年必须有可复现取值命令（`git log -S <锚点>` 取首现），否则 UNDATED 盲区只是被 DECIDED 换皮掩盖。r47 实测 12 条里 6 条无轮号可考，须如实写 `r?` 加日期而非假装完成 |
| X-20 | 拿真实记忆卷做冒烟测试 | **r46 立**：本轮用真实 07 试写工具，误落一条`r46 裁决=冒烟测试`（当场回滚）。新写入器的自证一律在临时 vault 做（`r46_mark_verdict_fixtures.py` 即如此），真实卷只接受正式裁决 |
| X-19 | 用行号索引写共享记忆卷的裁决/状态标记 | **r45 立**：W-9 挂 5 轮的真因就是它 —— 目标行裁决标记数为 0，说明三轮「按行号补标」全写到别的行（07 在拆卷与并发重写下行号必漂）。只准用 `r38_debt_aging.find_item_line()` 标题锚点，命中数 != 1 即拒写并报告 |
| X-18 | 判据/工具写完就宣布有效，不当场拿真实数据跑一遍 | **r44 立**：本轮 W-12 落地后首跑即报「上轮 15 条 OVERDUE 一条没裁」，证明「代码在」不等于「债还了」；r31–r44 反复出现该模式。落地判据的当轮必须出现一次真实取数与真实压降数字，否则条目不得标完成 |
| X-17 | 对 REPEAT 项（同一条已改期 >=2 次）**再挂一个新到期日** | **r43 立**：滚延期会让账面永远是 DEFERRED、趋势线看不出恶化；必须三选一 —— 执行 / 降级为长期看守（数值仍被棘轮看守）/ 作废（带实测证据） |
| X-15 | 判据读「同一实体的多次状态」时取**首个**标记（让最早那次决定永久生效） | **r41 立**：`RE_DECIDED.search()` 让新的终局裁决被旧挂账劫持，11 条到期项反复判红；改 `finditer()[-1]`（last-wins）并与 `gate_runs.jsonl` / `ac-verdicts.jsonl` 的 append-only 口径统一 |
...
## 排障速查（报错特征 → 根因 → 一行检查；r49–r54 本机实测固化，非推测）

> 用法：先看左列的原始报错串，命中即用右列命令定位，**先修判据/修接线，再动数据**。
> 本仓长期教训：多数"账面变红"是判据或接线的问题，不是数据坏了（R263/R220 同族）。
...
| 5 | 夹具全绿，真机跑出来是错的 | 夹具入参形态与**真实派发处不同**（r51：不变式实收名字串被我当时间戳 ⇒ 全量历史件被字典序静默豁免；r53：句柄在写台账前构建 ⇒ 解出上一行） | 新判据必须补"按真实传参/真实 CLI 跑通"的断言（r54 g10/g11、r53 h12/h13 即此形态） |
| 6 | 新判据首跑报红，但被报的对象根本不该归它管 | 单面识别：r52 建议面把 `X-24` 判成漏登记（X 的承接面是 AGENTS 禁止项表）；r54 陈旧自检按文件名判新鲜度，3 个红全是假阳性 | **新判据首跑双向验**：既造"该红的红"，也造"该绿的没被误伤"；零输入 ⇒ 记 UNVERIFIED 不记 OK |
| 7 | `[GATES:FAIL]` 只有 `gate_run_freshness` 一道红 | 元门读**上一行**台账，本轮修完仍延迟一轮（设计如此） | 再跑一次 `python 05-exec/run_gates.py` 即自愈；别为消除延迟去改台账 |
| 8 | `UnboundLocalError: cannot access local variable ...` | 两种：① 函数体内 `import X` 遮蔽模块级同名（r53 tempfile）② 引用了尚未构造的对象（r53 打印 `doc` 早于赋值） | AST 扫遮蔽与先用后赋：`python -c` 走 `ast.walk` 比对 Store/Use 行号（本轮做法已验证有效） |
| 9 | `SyntaxError: invalid syntax. Perhaps you forgot a comma?`（我刚写的代码） | 中文串里用了 ASCII 双引号把外层字符串截断；或 `\` 被 heredoc / `-c` / 工具参数吃掉 | 含引号或反斜杠的代码**落成 .py 再跑**（本仓第八形态）；写完先 `ast.parse` 自证再执行 |
...
| 13 | 照本表某条检查命令跑，报出一堆你从没见过的问题 | **检查命令自身的面不对**：r54 实测 `handoff.py noise <项目根>` 对本项目报 8 条 VIOL（`03-audit`/`04-plan`/`05-exec`/`06-benchmark`/`memory`/`archive` 全中），因为项目根不在 `KNOWN_CORE` allowlist 内 ⇒ 默认 strict 判所有目录。**✅ r55 已修**：受管根 `e93896b`（R286）把本项目根登记进 `KNOWN_CORE`，复验顶层 VIOL **8 → 0**、二级扫描随即露出 3 条真项（`手动维护。agent` + `_tmp_r54` + `mark_backup`）⇒ **登记不放宽判据**，只是把面认错纠正。**⚠️ 另一半由 r55 二次自抓补齐**：登记只治好**绝对路径**，`noise .`（相对写法）当时仍判 13 项 ⇒ **同一条命令的两种写法就是两个面**；受管根 `e5c785e`（R287）把查表键绝对化后两面同结论（夹具 f10/f11 常驻）。**残留口径差仍在**：savepoint 那句「✅ 零散射」不含项目根（见 row 1），**禁**把它当"项目根干净"的证据 | 先问「这条命令读的是什么面、用的哪种路径写法」再动手；隔离桩 `05-exec/r37_noise_tracked_stub.py` 可复跑；**禁**据此迁移或删除已跟踪目录凑绿（X-9） |
...
> 三条通用取向（比逐条更常用）：
> ① 判据报红先问"它读的面是全的吗"（覆盖面/下限/交叉/保质期四类自证都在 `05-exec/r38_debt_aging.py`、`r50_face_fixtures.py`、`r54_refresh_fixtures.py`）；
> ② 数字只写句柄不抄值（`python 05-exec/r38_debt_aging.py` 的「可引用句柄」行），源头断链必须显形为 `None`；
> ③ 破坏性动作先备份 + Git（本仓 pre-commit 链、受管根走 `rule_editor.py`），且**消不掉的告警等于没有判据**，要实减或显式重设并归因。
