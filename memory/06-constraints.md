...
- [DEBT] **无「重复工作识别」闸门：同一条指令被机器排程整点重放，本体系照单全收** — 实测（2026-09-24，r21）：本仓当日 11 个提交中 **5 轮属同一「对标」指令族**，贡献 **10,466 行 = 全仓新增 83.7%**；`lessons_usage.jsonl` 反查同日同指令 **4 个 session 且跨项目**（本仓 ×2、`qd-zijian-r19`、`qd-filialconnect-0924`）；根因指纹 = `~/.qoder-cn/logs/runs/…T{12..17}-00-00…` **整点等间隔 + 同一 PID + `query_source="sdk"` + `scheduled session_id=… cwd=…超市web`**（Qoder 内置定时会话轮转项目重放）。**二阶后果已实物化**：同一小时内出现两个不同内容的 r20（`ab1bb8c` 19:06 / `9782a9e` 19:42）⇒ 多会话对同一指令分叉入库，正是「跨会话夹带 / R 号撞号」两条既有缺陷的生成机制。对标：superpowers / spec-kit / AAS / addyosmani **四家皆有**确定性收敛或 fail-closed 闸门，本体系此前为**空列** | 影响范围：算力、记忆分叉、门禁夹带三条同时受损 | 还债方式：① 已落 `05-exec/repeat_round_guard.py`（FRESH/THIN/INCREMENT→0、DUPLICATE→1、取证失败→2 按 R247；两层含对照实测）；② **排程侧接线与源头降频属用户决策面，本机无配置可改**（Windows 计划任务与明文配置均已实测排除）；取值命令 `ls ~/.qoder-cn/logs/runs | grep -E "T[0-9]{2}-00-0" | wc -l` | 状态：**未修复**（工具就绪，源头未断）
