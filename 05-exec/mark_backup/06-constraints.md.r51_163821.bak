...
- [BUG] **记忆卷混合行尾 ⇒ 任何「按整份单一行尾切分」的写入器都会并元素错写** — 影响范围：`memory/07-next-steps.md` 实测 178 个 CRLF + 4 个纯 LF（并发工具按 text 模式写入的产物）。2026-09-25 r49 实测：`r46_mark_verdict.py` 首版按「文件里有 
 就整份按 CRLF 切」，那 4 行被并进前一元素，标记追加到元素末尾 = **另一条目那一行**（W-16 的裁决落到 W-14 行）。已修写入侧（`split_keep_eol` 逐行保留 + 同行读回断言 + 反例夹具 t9/t10），**根因仍在**：那 4 行由谁写入未定位，统一行尾属 R241 敏感区（禁为让门变绿而整卷重写）⇒ 登记为 W-26 看守 | 状态：**未修复（已缓解）** —— **✅ 归因校正注（2026-09-25 r50，实测非推测）**：原句把成因写成「并发工具按 text 模式写入」当时**未经核验**。实测口径 = `git show r43..r49:memory/07-next-steps.md` 各历史版本**全部行均为纯 LF**（r49 版 190 行、0 个 CRLF），而工作树为 187 CRLF + 4 纯 LF ⇒ 混合行尾由**本仓写卷脚本未传 `newline=""`** 造成（Windows text 模式把 LF 前补 CR），与并行会话无关。判据侧已按物理行处理（W-23）故不再受影响；写入侧约定立为 X-25。原句保留不改写（R241）。
