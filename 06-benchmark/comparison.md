...
> 取值（r52 补，原句不改）：`python 05-exec/r38_debt_aging.py --json /tmp/x.json` 读 by_class，逐条裁决见 `06-benchmark/debt_aging_r38_2026-09-25.json` 的 overdue 清单与当日 GM 日志 r38 块
> 对手消灭债务靠"关掉"，我靠"逼人裁决"，因为 07 是跨会话唯一真相源。**可宣称领先 4/5 对手，不可宣称领先 spec-kit。**

> **r39 追加（第十四维纵深：延期与终局拆分）**：账龄判据补 **DEFERRED** 独立态 —— 「挂账至 rNN」不再算已裁决，
> 目标轮一到就**回判 OVERDUE（到期追讨）**；趋势台账 `06-benchmark/debt_runs.jsonl` 三重 fail-closed 拒写，注册为第 9 道常驻门（夹具 19/19）。
> 与对手的关键分界：`github/spec-kit` 的 stale bot **到期即关闭（等于把债务标为完成）**，
> 我方**到期只升级判罚、不消灭债务**。本轮 W-0 实测 `OVERDUE 22 → 0`，同时 `DEFERRED 23` 单列在册（未销账，基线 note 已记敞口）。

> **r42 追加（还债轮第三击）**：`OVERDUE 1 → 20` 的反弹经四阶段调试定性为**流程缺陷而非判据缺陷**：
> 14 条是我 r39 写的 `挂账至 r41` **如期到期**，6 条是我**漏裁**（只裁当期到期清单、没裁账龄新越线清单）。
> 取值（r52 补，原句不改）：`python 05-exec/r38_debt_aging.py` 比对两份证据件的 overdue 集合差（r41 件 = 06-benchmark/debt_aging_r41_2026-09-25.json；r42 反弹记录见当日报告 r42 §1）
> 由此立 X-16 + 两条待落地判据：`REPEAT`（同一条改期 >=2 次必须显形，反延期 treadmill）与
> `W-12` 裁决清单完整性（本轮裁决数 == 上轮 OVERDUE 数，缺一条即红）。失败测试 t30/t32/t33/t34 已入库（28/32），实现待 r43。
> 对手侧不变：仅 `spec-kit` 有到期自动化且**到期即关闭**；其余四家 0 条。我方取向是到期升级判罚、不消灭债务。

> **r43 追加（第十四维第四击：反延期 treadmill）**：新增第 6 态 `REPEAT`（同一条目被改期 >=2 次即显形）
> 与**按优先级分档宽限** `P0:2 / P1:4 / P2:8`。首跑抓到 6 条在滚延期（含我 r39 批量写的那批），
> 分档后 OVERDUE 20→15；随后按 X-16 把已延期过的 11 条判「降级为长期看守」（终局态但数值仍被棘轮看守），
> 首次越线 4 条给到期日 r44 之外的 r46。**棘轮仍红（+15）且未上调基线** —— 对手五家均无「反复改期」度量，此面本轮起领先。

> **r44 追加（第十四维第五击：裁决完整性）**：adjudication_gap 把「上轮 OVERDUE 本轮是否真被处理」变成机器判据，
> 首跑即报 INCOMPLETE｜15 → 15（我当时正准备把 W-12 标完成）；据此真裁决 15 条后 OVERDUE 16→2→1、DECIDED 43。
> 语义由夹具 t40–t46 锁死：无标记或最后一次挂账已到期 ⇒ missing；多命中逐行计（防只裁第一行）；已勾/已消失/未来挂账不算。
> 立 **X-18**（判据不当场跑真实数据不得宣布有效）与 **W-14**（REPEAT 第 9 指标，防「降级」成新免检通道）。
> 对手侧五家均无「清偿率」度量（anthropics 1290 / superpowers 401 / spec-kit 287 / agent-skills 118 / pre-commit 25 open 只增不减）。

> **r45 追加（第十四维第六击：反降级免检）**：新增棘轮第 9 指标 `repeat_debt_items`（读 REPEAT），
> 动因是 r44 用 11 条「降级为长期看守」把 OVERDUE 从 16 压到 1 —— 降级合法，但**没有独立指标的降级就是新免检通道**。
> 同时把 W-9「鬼影项」查到真因：该行**裁决标记数为 0**，证明前几轮我按行号写标记写到了别的行（W-13 的实证据，立 W-16）。
> 对手侧：五家均无 REPEAT/降级度量；`spec-kit` 到期即关闭标签，等于用关闭替代还债。
> 本轮 OVERDUE 3→0、DECIDED 46、REPEAT 0，W-12 完整性首报 **OK**；契约侧未纳入 coverage 列的原因与后续方案见 W-17。

> **r46 追加（第十四维第七击：写侧固化）**：约定拦不住自己（r45 立 X-19 当轮就违反），故把「禁按行号写标记」做成唯一接口
> `r46_mark_verdict.py`：不收行号、歧义/0 命中/已闭环/同轮二次一律拒写、写完必须读回否则回滚、备份可注入。
> 真实卷冒烟当场拒写歧义锚点 `[87,90]` —— 这就是 W-9 挂 5 轮的机制学解释。注册为第 11 道门（PASS=11），dogfood 裁掉 W-6 ⇒ OVERDUE 1→0。
> 对手五家均为**纯写侧自由格式**（issue 正文手写、无锚点约束），此面本体系独有；三处自抓（备份目录写死、弱断言 t9、真实卷冒烟误写）见报告 D66-D68。

> **r47 追加（还债轮：定年盲区清零）**：12 条 UNDATED 逐条以 `git log -S` 取真实首现、经写入器落「定年 + 终局裁决」，
> `UNDATED 12→0`、`DECIDED 58`、`OVERDUE 0`。对手台账没有「无法定年」这一格，是因为它们本就没有跨会话账龄概念 ——
> 我方本轮起债务台账**无年龄盲区**，且不夸大：12 条里 6 条只定得到日期（首现提交无 rNN），如实写 `r?`。
> 立 X-21（禁以裁决标记冒充定年）。W-18 首次全程 dogfood：12/12 经写入器，零行号错写、备份不落记忆卷。

> **r48 追加（契约分代 + 归属可机检）**：`row_required_from` 让台账新列可按 `ts` 分叉强制而不误杀 append-only 历史行（解 r45 自挂两难）；
> `classify_owner()` 把「这不属于我」变成判据 —— 声明归属却给不出可解析路径即 `UNOWNED`（动因：r40 我把自家件标成归属方件挂 4 轮）。
> 首跑自抓假阳性：owner 用截断 `title[:110]` 判，3 条里 2 条是假的 ⇒ 改判全文，剩 3 条真项补真实路径后 UNOWNED 3→0（最终复测值）（夹具 53/53）。
> 对手侧无此层：开源仓的 issue assignee 是**人**，不需要「甩锅可机检」；本体系是多会话共享卷，甩锅成本为零才是真风险。
