...
> 生成：2026-09-24 09:32 ｜ 扫描根：`D:\global_skills` ｜ 工具：`05-exec/description_baseline_scan.py`（只读）
...
| prompt-system-audit | 34 | ✗ | 多agent提示词系统逻辑一致性审计 + 数据层验证。提示词系统审计 |
| agent-browser | 36 | ✗ | 浏览器自动化CLI — AI agent时代浏览器操控。浏览器自动化工具 |

## 无触发标记（何时用缺失，48 条，前 20 按长度升序）
