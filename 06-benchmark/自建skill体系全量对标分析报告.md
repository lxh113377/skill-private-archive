...
> **⚠️ 校正注（2026-09-24 r27，R241 只加注）**：正文（2026-09-23）中所有以 `151` 为分母的统计、以及 `C1~C23` / 五端口径，现均已失效。取值命令 `python 焚诀/eval/verify_truth_consistency.py`：当前注册表 == 磁盘 **167**、门禁 **C1~C33**、**八端**。本报告 G1~G5 已于 r17/r17b 闭环或转归属会话；增量账见 `全量对标报告_r18_2026-09-24.md`（N1~N8）与 `全量对标报告_r27_2026-09-24.md`（G1~G6）。
